...
namespace PixelCrushers.DialogueSystem.MenuSystem
{
    ...
    public class MenuUtility : MonoBehaviour
    {
        ...
        private IEnumerator ReturnToTitleWhenDoneSaving()
        {
            ...
            if (saveHelper != null)
            {
                Debug.Log("SaveHelper found: " + saveHelper.gameObject.name);

                // You can access properties or methods of the saveHelper here.
            }
            else
            {
                Debug.Log("SaveHelper not found in the scene.");
            }
            Debug.Log("made it to here");
